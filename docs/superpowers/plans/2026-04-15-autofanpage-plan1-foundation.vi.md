# AutoFanpage — Plan 1: Foundation + Vertical Slice (Bản tiếng Việt)

> **Lưu ý:** Đây là bản dịch tiếng Việt của `2026-04-15-autofanpage-plan1-foundation.md` để review dễ hơn. File tiếng Anh là bản gốc dùng cho agentic workers. Toàn bộ code, file path, command, JSON schema được giữ nguyên.

> **Cho agentic workers:** BẮT BUỘC dùng SUB-SKILL: superpowers:subagent-driven-development (khuyến nghị) hoặc superpowers:executing-plans để thực thi plan này theo từng task. Các bước dùng cú pháp checkbox (`- [ ]`) để tracking.

**Mục tiêu:** Dựng package skill AutoFanpage trên OpenClaw với một vertical slice (lát dọc end-to-end) tối giản: orchestrator skill → 1 data-source skill (Hacker News) → Telegram reporter. Mục đích là chứng minh pattern gọi skill trên OpenClaw, cơ chế load profile, hợp đồng run-directory, và Telegram reporting đều hoạt động đồng bộ — trước khi bổ sung các tích hợp phức tạp hơn.

**Kiến trúc:** Python package `autofanpage/` nằm ở thư mục gốc của project, chứa các thư viện dùng chung (profile loader, run-directory manager, schema validator, secrets wrapper, sub-skill dispatcher). Các thư mục skill nằm trong `skills/`, mỗi thư mục chứa file SKILL.md cho OpenClaw và các script Python trong `skills/<name>/scripts/`. Test nằm trong `tests/` và dùng pytest. Các skill phát triển trong repo được copy (hoặc symlink) vào `~/.openclaw/skills/autofanpage/` để chạy runtime qua script `scripts/install.sh`.

**Tech Stack:** Python 3.11+, pytest, `jsonschema` để validation, `requests` cho HTTP, OpenClaw CLI (`openclaw skills run`, `openclaw secrets get`) được gọi qua `subprocess` từ một dispatcher có thể thay thế khi test. Không dùng framework nào khác.
...
**Python package (`autofanpage/`) — thư viện dùng chung cho mọi skill:**
- `autofanpage/__init__.py` — rỗng
- `autofanpage/profile.py` — `load_profile(path) -> Profile` với validation
- `autofanpage/run_dir.py` — class `RunDir`: tạo/resolve run dir, đọc/ghi các file JSON artifact
- `autofanpage/state.py` — class `LastSuccess`: đọc/ghi `state/<page>/last_success.json`
- `autofanpage/schemas.py` — các JSON schema fragment cho mọi artifact, cùng hàm `validate(name, data)`
- `autofanpage/secrets.py` — `get_secret(ref) -> str`; backend mặc định là subprocess gọi `openclaw secrets get`, có thể thay thế khi test
- `autofanpage/dispatch.py` — `run_skill(name, args_dict) -> dict`; backend mặc định là subprocess gọi `openclaw skills run`, có thể thay thế khi test
- `autofanpage/telegram.py` — wrapper mỏng dùng bởi `telegram-reporter`
- `autofanpage/errors.py` — các exception có kiểu rõ ràng (`ProfileError`, `SchemaError`, `SkillInvocationError`, `AlreadyRanError`)

**Thư mục skill (`skills/`) — được đưa lên `~/.openclaw/skills/autofanpage/`:**
...
**Tests (`tests/`):**
- `tests/conftest.py` — các fixture dùng chung
...
- `scripts/install-skills.sh` — copy `skills/*` vào `~/.openclaw/skills/autofanpage/`
...
- [ ] **Step 4: Viết package init rỗng + tests init + conftest**
...
Chạy: `pytest`
Mong đợi: `no tests ran in 0.XXs` (exit 5 là bình thường — chưa có test nào).
...
### Task 2: Typed exceptions (các exception có kiểu)

**Files:**
- Tạo: `autofanpage/errors.py`
...
Chạy: `pytest tests/test_errors.py -v`
Mong đợi: `ModuleNotFoundError: No module named 'autofanpage.errors'`

- [ ] **Step 3: Viết `autofanpage/errors.py`**
...
### Task 3: Helper validation JSON schema

**Files:**
- Tạo: `autofanpage/schemas.py`
...
Mong đợi: `ModuleNotFoundError: No module named 'autofanpage.schemas'`

- [ ] **Step 3: Viết `autofanpage/schemas.py`**
...
### Task 4: Profile loader (bộ đọc profile)

**Files:**
- Tạo: `autofanpage/profile.py`
- Tạo: `tests/fixtures/page_test.json`
- Test: `tests/test_profile.py`

- [ ] **Step 1: Viết fixture mẫu `tests/fixtures/page_test.json`**
...
- [ ] **Step 2: Viết test thất bại `tests/test_profile.py`**
...
Chạy: `pytest tests/test_profile.py -v`
Mong đợi: `ModuleNotFoundError: No module named 'autofanpage.profile'`

- [ ] **Step 4: Viết `autofanpage/profile.py`**
...
### Task 5: Run directory manager (quản lý thư mục chạy)

**Files:**
- Tạo: `autofanpage/run_dir.py`
...
Chạy: `pytest tests/test_run_dir.py -v`
Mong đợi: `ModuleNotFoundError: No module named 'autofanpage.run_dir'`

- [ ] **Step 3: Viết `autofanpage/run_dir.py`**
...
### Task 6: State / idempotency manager (quản lý trạng thái / tính idempotent)

**Files:**
- Tạo: `autofanpage/state.py`
...
Chạy: `pytest tests/test_state.py -v`
Mong đợi: `ModuleNotFoundError`.

- [ ] **Step 3: Viết `autofanpage/state.py`**
...
### Task 7: Secrets wrapper (wrapper lấy secret)

**Files:**
- Tạo: `autofanpage/secrets.py`
...
Chạy: `pytest tests/test_secrets.py -v`
Mong đợi: `ModuleNotFoundError`.

- [ ] **Step 3: Viết `autofanpage/secrets.py`**
...
### Task 8: Sub-skill dispatcher (bộ điều phối sub-skill)

**Files:**
- Tạo: `autofanpage/dispatch.py`
...
Chạy: `pytest tests/test_dispatch.py -v`
Mong đợi: `ModuleNotFoundError`.

- [ ] **Step 3: Viết `autofanpage/dispatch.py`**
...
### Task 9: Logic lấy bài Hacker News (hàm thuần — không gọi mạng khi test)

**Files:**
- Tạo: `autofanpage/sources/__init__.py`
- Tạo: `autofanpage/sources/hackernews.py`
- Test: `tests/sources/test_hackernews.py`
- Fixture test: `tests/fixtures/hn_items.json`

- [ ] **Step 1: Tạo fixture `tests/fixtures/hn_items.json`**
...
- [ ] **Step 2: Viết test thất bại `tests/sources/test_hackernews.py`**

Tạo `tests/sources/__init__.py` (rỗng), sau đó:
...
Chạy: `pytest tests/sources/test_hackernews.py -v`
Mong đợi: `ModuleNotFoundError`.

- [ ] **Step 4: Viết `autofanpage/sources/__init__.py`** (rỗng) **và `autofanpage/sources/hackernews.py`**
...
### Task 10: Hacker News network fetcher (script gọi mạng cho skill)

**Files:**
- Tạo: `skills/hackernews-researcher/SKILL.md`
- Tạo: `skills/hackernews-researcher/scripts/__init__.py`
- Tạo: `skills/hackernews-researcher/scripts/fetch_hn.py`
- Test: `tests/skills/test_hackernews_fetch.py`

- [ ] **Step 1: Viết integration test thất bại `tests/skills/test_hackernews_fetch.py`**

Tạo `tests/skills/__init__.py` (rỗng), sau đó:
...
Chạy: `pytest tests/skills/test_hackernews_fetch.py -v`
Mong đợi: `ModuleNotFoundError: No module named 'fetch_hn'`.

- [ ] **Step 3: Viết scaffold cho skill**

`skills/hackernews-researcher/scripts/__init__.py`: rỗng.
...
- [ ] **Step 5: Viết `skills/hackernews-researcher/SKILL.md`**
...
**Files:**
- Tạo: `autofanpage/telegram.py`
- Test: `tests/test_telegram.py`
- Tạo: `skills/telegram-reporter/SKILL.md`
- Tạo: `skills/telegram-reporter/scripts/report.py`
- Test: `tests/skills/test_telegram_report.py`

**Lưu ý về transport:** Kênh Telegram native của OpenClaw gửi tin nhắn bằng cách in ra stdout trong agent turn (theo OpenClaw "Chat Integrations" — xác nhận cơ chế chính xác khi smoke test lần đầu). Trong khi chờ xác nhận, skill này ghi thông báo cả ra stdout (dạng JSON) VÀ vào file log `run_dir/telegram_sent.log`, để có thể kiểm tra thủ công ngay cả khi kết nối kênh OpenClaw chưa hoàn hảo trong lần đầu.

- [ ] **Step 1: Viết test thất bại cho việc format template**
...
Chạy: `pytest tests/test_telegram.py -v`
Mong đợi: `ModuleNotFoundError`.

- [ ] **Step 3: Viết `autofanpage/telegram.py`**
...
- [ ] **Step 5: Viết test thất bại cho entry point của skill**
...
Chạy: `pytest tests/skills/test_telegram_report.py -v`
Mong đợi: `ModuleNotFoundError: No module named 'report'`.

- [ ] **Step 7: Viết `skills/telegram-reporter/scripts/report.py`**
...
- [ ] **Step 9: Viết `skills/telegram-reporter/SKILL.md`**
...
### Task 12: Orchestrator skill (vertical slice HN + Telegram)

**Files:**
- Tạo: `skills/daily-content-pipeline/SKILL.md`
- Tạo: `skills/daily-content-pipeline/scripts/orchestrate.py`
- Test: `tests/skills/test_orchestrator.py`

- [ ] **Step 1: Viết test end-to-end thất bại**
...
Chạy: `pytest tests/skills/test_orchestrator.py -v`
Mong đợi: `ModuleNotFoundError: No module named 'orchestrate'`.

- [ ] **Step 3: Viết `skills/daily-content-pipeline/scripts/orchestrate.py`**
...
- [ ] **Step 5: Viết `skills/daily-content-pipeline/SKILL.md`**
...
### Task 13: Install script (đồng bộ `skills/` vào OpenClaw)

**Files:**
- Tạo: `scripts/install-skills.sh`

- [ ] **Step 1: Viết install script**
...
```
Mong đợi output có: `daily-content-pipeline  hackernews-researcher  telegram-reporter`

- [ ] **Step 4: Dọn dẹp**

Chạy: `rm -rf /tmp/fake-openclaw-skills`
...
### Task 14: Tài liệu smoke test thủ công

**Files:**
- Tạo: `docs/superpowers/smoke-tests/plan1.md`

- [ ] **Step 1: Viết tài liệu smoke test**
...
### Task 15: Toàn bộ test suite xanh + kiểm tra coverage

- [ ] **Step 1: Chạy toàn bộ test suite**

Chạy: `pytest -v`
Mong đợi: Tất cả test từ Task 2–12 đều pass. Khoảng 25+ test.

- [ ] **Step 2: Kiểm tra coverage của thư viện core**

Chạy: `pytest --cov=autofanpage --cov-report=term-missing`
Mong đợi: Coverage của package `autofanpage/` đạt ≥ 85%. Các dòng chưa được cover nên giới hạn ở subprocess backend (được kiểm tra thủ công trong smoke test) và các error path khó đạt được khi không có OpenClaw thật.

- [ ] **Step 3: Nếu coverage < 85%, bổ sung test có mục tiêu**

Xác định các nhánh chưa cover từ output của Step 2 và thêm test. Commit riêng:
```bash
git add tests/
git commit -m "test: lift autofanpage coverage above 85%"
```

- [ ] **Step 4: Tag milestone Plan 1**
...
Độ phủ spec (chỉ phạm vi Plan 1):
- §2 bố cục file — được implement trong Task 1, 10, 11, 12 (thư mục skill + python package + bố cục dữ liệu runtime qua `RunDir` và `LastSuccess`).
- §3.1 orchestrator — Plan 1 implement bước 1–3 (load profile, xác định ngày, kiểm tra idempotency) cộng nhánh HN của bước 4. Các bước còn lại (merge Perplexity/YouTube/Reddit, NotebookLM, Review/Writing/Publisher) cố ý hoãn sang Plan 2–4.
- §3.5 hackernews-researcher — Task 9 (logic thuần) + Task 10 (network + kết nối skill).
- §3.10 telegram-reporter — Task 11 (format + kết nối skill). Transport qua kênh native OpenClaw được xác nhận ở smoke test (Task 14).
- §3.11 autofanpage-health-check — hoãn sang Plan 4.
- §4.1 profile schema — Task 3 schemas + Task 4 profile loader.
- §4.3 last_success.json — Task 6.
- §5 xử lý lỗi — orchestrator bắt `AutofanpageError` + `Exception` và route cả hai đến `telegram-reporter` với `status=error`. Các hàng lỗi cụ thể trong §5 cho YouTube/Perplexity/Reddit/NotebookLM v.v. sẽ được bổ sung dần trong Plan 2–4 khi các skill đó được implement.
- §6 testing — unit test xuyên suốt, integration test trong Task 10, orchestrator E2E test trong Task 12, smoke test thủ công trong Task 14.
- §7 deployment — `install-skills.sh` trong Task 13; deployment checklist đầy đủ hoãn sang Plan 4 vì phụ thuộc vào các skill chưa implement.
- §8 câu hỏi mở — Câu hỏi 1 (cú pháp gọi sub-agent OpenClaw) được trả lời một phần bởi giả định `SubprocessBackend`. Smoke test trong Task 14 là xác nhận thực tế đầu tiên; nếu giả định sai, chỉ cần sửa `autofanpage/dispatch.py` — không có code nào khác phụ thuộc vào cơ chế transport.

Không có placeholder nào. Các kiểu dữ liệu nhất quán (`Profile`, `RunDir`, `LastSuccess`, `SchemaError`, `SkillInvocationError` được dùng cùng cách trên tất cả task).

---

## Bàn giao thực thi

Plan đã hoàn chỉnh và lưu tại `docs/superpowers/plans/2026-04-15-autofanpage-plan1-foundation.md`. Hai lựa chọn thực thi:

**1. Subagent-Driven (khuyến nghị)** — Dispatch một subagent mới cho mỗi task, review giữa các task, vòng lặp nhanh.

**2. Inline Execution** — Thực thi task trong session này dùng executing-plans, thực thi theo lô với các checkpoint.
